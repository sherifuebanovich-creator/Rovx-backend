...
**Not changed (deferred, needs a TomTom API key the user will provide later):** the "traffic shown as circles instead of lines along the road, like Yandex" complaint — `TrafficFlowLayer.tsx` already implements exactly that (TomTom traffic-flow raster tiles), it's just inert because `NEXT_PUBLIC_TOMTOM_API_KEY` is unset. The circles users actually see are a different layer (`TrafficLayer.tsx`, community-submitted point reports) — nothing to fix there without real road-segment geometry, which report data doesn't have.
