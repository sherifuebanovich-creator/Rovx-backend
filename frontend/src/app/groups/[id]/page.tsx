...
import {
  ...
  FaSignOutAlt, FaCalendarAlt, FaGlobe, FaLock, FaChevronRight,
} from 'react-icons/fa';
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] ${msg.sticker ? 'text-5xl' : `px-4 py-2.5 rounded-2xl ${
              ...
              {/* Sticker */}
              {msg.sticker && (
                <span className="block">{msg.sticker}</span>
              )}
                  ...
                  <div className="space-y-2">
                    {group.members?.map(m => (
                      <div key={m.id} className="flex items-center gap-3 py-1">
                        ...
                        {m.isAdmin && (
                          <FaShieldAlt size={12} className="text-primary-400 flex-shrink-0" />
                        )}
                        {m.userId === group.ownerId && (
                          <FaCrown size={12} className="text-yellow-400 flex-shrink-0" />
                        )}
                      </div>
                    ))}
                  </div>
